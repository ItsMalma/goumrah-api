import { Hono } from "hono";
	...
	const foodMenuController = new Hono()
		...
		.get("/:id", validate("param", foodMenuParamSchema), async (c) => {
			...
			const output = await FoodMenuService.instance.getById(param, true);
		...
		.put(
			...
			async (c) => {
				...
				const output = await FoodMenuService.instance.update(
					param,
					input,
					true,
				);
		...
		.delete("/:id", validate("param", foodMenuParamSchema), async (c) => {
			...
			const output = await FoodMenuService.instance.delete(param, true);
